...
| **Domain-Specific** | R4 UI Toolkit | Window Compositing Layer Order Law (macOS), Window Decoupling Law (a Window is just a Window), Pane-of-Glass Law, Continuous Real-Time Live Resize & Presentation Law (Abolishing "Freeze-Exact"), Native Pixel Law, Panel Gravity Law, Sub-Part Field Segregation Law & the `Class_part_verb` Law, Living Darling Docs Law (Zero Drift Between Code and `_docs/darling.md`), Ecosystem Vulkan Safety Nets Law (Determinism + Tree-Shaken Truth), Window Board Root Lock Law (Dimension Override Law) |
...
### Window Compositing Layer Order Law (macOS)
